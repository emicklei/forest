...
func TestXMLPathWrongPath(t *testing.T) {
	...
	if m.errorMessage != "XMLPath: no value for path: /Root/Child/ValueX" {
...
func TestXMLPathInvalidPath(t *testing.T) {
	...
	m := new(mockedT)
	XMLPath(m, r, "/{Root}")
	if m.errorMessage != "XMLPath: invalid xpath expression:compiling xml path \"/{Root}\":1: missing name" {
		t.Errorf("expected other message than:[%s]", m.errorMessage)
	}
}
...
func TestXMLPathNoResponse(t *testing.T) {
	m := new(mockedT)
	XMLPath(m, nil, "/Root")
	if m.fatalMessage != "XMLPath: no response to read body from" {
...
func TestXMLPathNoBody(t *testing.T) {
	...
	r := new(http.Response)
	XMLPath(m, r, "/Root")
	if m.fatalMessage != "XMLPath: no response body to read" {
		t.Errorf("expected other message than:[%s]", m.fatalMessage)
	}
}
...
func TestXMLPathNoDocument(t *testing.T) {
	...
	m := new(mockedT)
	XMLPath(m, r, "/Root")
	if m.errorMessage != "XMLPath: no value for path: /Root" {
		t.Errorf("expected other message than:[%s]", m.errorMessage)
	}
}
